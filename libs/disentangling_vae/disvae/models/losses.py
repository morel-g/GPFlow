...
class BaseLoss(abc.ABC):
    ...
    def __init__(
        self, record_loss_every=50, rec_dist="bernoulli", steps_anneal=0
    ):
    ...
    @abc.abstractmethod
    def __call__(
        self, data, recon_data, latent_dist, is_train, storer, **kwargs
    ):
        """
...
class BtcvaeLoss(BaseLoss):
    ...
    def __init__(
        self, n_data, alpha=1.0, beta=6.0, gamma=1.0, is_mss=True, **kwargs
    ):
        super().__init__(**kwargs)
        self.n_data = n_data
        self.beta = beta
...
def _reconstruction_loss(
    data, recon_data, distribution="bernoulli", storer=None
):
    """
...
def _get_log_pz_qz_prodzi_qzCx(
    latent_sample, latent_dist, n_data, is_mss=True
):
