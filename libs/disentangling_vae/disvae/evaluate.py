...
class Evaluator:
    ...
    def __call__(self, data_loader, is_metrics=False, is_losses=True):
        ...
        self.logger.info(
            "Finished evaluating after {:.1f} min.".format(
    ...
    def compute_losses(self, dataloader):
        ...
        for data, _ in tqdm(
            dataloader, leave=False, disable=not self.is_progress_bar
        ):
    ...
    def _axis_aligned_metric(self, sorted_mut_info, storer=None):
        ...
        numerator = (
            sorted_mut_info[:, 0] - sorted_mut_info[:, 1:].sum(dim=1)
        ).clamp(min=0)
    ...
    def _compute_q_zCx(self, dataloader):
        ...
        with torch.no_grad():
            for x, label in dataloader:
                ...
                q_zCx[idcs, :, 0], q_zCx[idcs, :, 1] = self.model.encoder(
                    x.to(self.device)
                )
                n += batch_size
    ...
    def _estimate_latent_entropies(
        self, samples_zCx, params_zCX, n_samples=10000
    ):
        ...
        mean = (
            params_zCX[0]
            .unsqueeze(-1)
            .expand(len_dataset, latent_dim, n_samples)
        )
        log_var = (
            params_zCX[1]
            .unsqueeze(-1)
    ...
    def _estimate_H_zCv(self, samples_zCx, params_zCx, lat_sizes, lat_names):
        ...
        for i_fac_var, (lat_size, lat_name) in enumerate(
            zip(lat_sizes, lat_names)
        ):
            idcs = [slice(None)] * len(lat_sizes)
            for i in range(lat_size):
                self.logger.info(
                    "Estimating conditional entropies for the {}th value of {}.".format(
                        i, lat_name
                    )
                )
                idcs[i_fac_var] = i
                ...
                samples_zxCv = (
                    samples_zCx[idcs]
                    .contiguous()
                    .view(len_dataset // lat_size, latent_dim)
                )
                params_zxCv = tuple(
                    p[idcs]
                    .contiguous()
                ...
                H_zCv[i_fac_var] += (
                    self._estimate_latent_entropies(samples_zxCv, params_zxCv)
                    / lat_size
                )
        return H_zCv
