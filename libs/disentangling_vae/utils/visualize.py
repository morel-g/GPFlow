...
class Visualizer:
    def __init__(
        ...
        self.output_dir = output_dir if output_dir is not None else model_dir

        self.dataset = dataset
        self.upsample_factor = upsample_factor
        if loss_of_interest is not None:
            self.losses = read_loss_from_file(
                os.path.join(self.model_dir, TRAIN_FILE), loss_of_interest
            )
    ...
    def _traverse_line(self, idx, n_samples, data=None):
        ...
        if data is None:
            ...
            traversals = torch.linspace(
                *self._get_traversal_range(), steps=n_samples
            )
            ...
            with torch.no_grad():
                # interp = not True
                # if interp:
                #     post_mean, post_logvar = self.model.vae.encoder(
                #         data.to(self.device)
                #     )
                #     samples = self.model.vae.reparameterize(
                #         post_mean, post_logvar
                #     )
                # else:
                post_mean, post_logvar = self.model.encoder(
                    data.to(self.device)
                )
                ...
                """
                    ...
                    # if post_logvar is None:
                    # Assume model ExtendVAE
                    # post_logvar = torch.ones_like(post_mean)
                """
                ...
                # traversals = torch.linspace(
                #     *self._get_traversal_range(
                #         mean=post_mean_idx, std=post_std_idx
                #     ),
                #     steps=n_samples
                # )
                traversals = torch.linspace(-1.75, 1.75, steps=n_samples)
        for i in range(n_samples):
            samples[i, idx] = traversals[i]
        # if interp and not isinstance(self.model, VAE):
        #     samples = self.model.interp(samples, idx)
    ...
    def _save_or_return(self, to_plot, size, filename, is_force_return=False):
        ...
        kwargs = dict(
            nrow=size[1], pad_value=(1 - get_background(self.dataset))
        )
    ...
    def reconstruct(
        self, data, size=(8, 8), is_original=True, is_force_return=False
    ):
    ...
    def traversals(
        ...
        """
        n_latents = (
            n_latents if n_latents is not None else self.model.latent_dim
        )
        latent_samples = [
        ...
        if is_reorder_latents:
            ...
            decoded_traversal = sort_list_by_other(
                decoded_traversal, self.losses
            )
        ...
        decoded_traversal = decoded_traversal[
            range(n_per_latent * n_latents), ...
        ]
    ...
    def reconstruct_traverse(
        ...
        name=PLOT_NAMES["reconstruct_traverse"],
    ):
        """
        ...
        if show_reconstruct:
            concatenated = np.concatenate(
                (reconstructions, traversals), axis=0
            )
        else:
            concatenated = traversals
        concatenated = Image.fromarray(concatenated)

        if is_show_text:
            losses = sorted(self.losses, reverse=True)[:n_latents]
            labels = ["orig", "recon"] if show_reconstruct else []
            labels += ["KL={:.4f}".format(l) for l in losses]
            concatenated = add_labels(
                concatenated, labels, right_labels=is_right_text
            )
    ...
    def gif_traversals(
        ...
        n_latents=None,
        n_per_gif=15,
        name=PLOT_NAMES["gif_traversals"],
    ):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.visualizer = Visualizer(
            model, dataset, model_dir, save_images=False, **kwargs
        )
