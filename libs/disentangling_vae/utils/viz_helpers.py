...
def get_samples(dataset, num_samples, idcs=[]):
    ...
    idcs += random.sample(
        range(len(data_loader.dataset)), num_samples - len(idcs)
    )
...
def read_loss_from_file(log_file_path, loss_to_fetch):
    ...
    df_last_epoch_loss.loc[:, LOSS] = (
        df_last_epoch_loss.loc[:, LOSS]
        .str.replace(loss_to_fetch, "")
        .astype(int)
    )
...
def add_labels(input_image, labels, right_labels=True):
    ...
    """
    width_shift = 100 if right_labels else 80
    new_width = input_image.width + width_shift
    new_size = (new_width, input_image.height)
    new_img = Image.new("RGB", new_size, color="white")
    horizontal_shift = 0 if right_labels else width_shift
    new_img.paste(input_image, (horizontal_shift, 0))
    draw = ImageDraw.Draw(new_img)

    for i, s in enumerate(labels):
        horizontal_pos = (
            new_width - width_shift + 0.005 if right_labels else 10.0
        )
        draw.text(
            xy=(
                horizontal_pos,
                int(
                    (i / len(labels) + 1 / (2 * len(labels)))
                    * input_image.height
                ),
            ),
            text=s,
...
def concatenate_pad(arrays, pad_size, pad_values, axis=0):
    ...
    pad = (
        np.ones_like(arrays[0]).take(indices=range(pad_size), axis=axis)
        * pad_values
    )
