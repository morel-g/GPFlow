    ...
    while i < num_points:
        ...
        if i == 0:
            ...
            representations = np.vstack(
                (
                    representations,
                    representation_function(current_observations),
                )
            )
        i += num_points_iter
...
def split_train_test(observations, train_percentage):
    ...
    num_labelled_samples_test = (
        num_labelled_samples - num_labelled_samples_train
    )
    observations_train = observations[:, :num_labelled_samples_train]
    observations_test = observations[:, num_labelled_samples_train:]
    assert (
        observations_test.shape[1] == num_labelled_samples_test
...
@gin.configurable("discretizer", blacklist=["target"])
def make_discretizer(
    target, num_bins=gin.REQUIRED, discretizer_fn=gin.REQUIRED
):
