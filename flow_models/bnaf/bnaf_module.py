# Source: https://github.com/nicola-decao/BNAF
...
class BNAFModule(torch.nn.Sequential):
    """
    ...
    def __init__(self, *args, res: str = None):
        ...
        *args : ``Iterable[torch.nn.Module]``, required.
        ...
        super(BNAFModule, self).__init__(*args)

        self.res = res

        if res == "gated":
            self.gate = torch.nn.Parameter(
                torch.nn.init.normal_(torch.Tensor(1))
            )
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.res == "normal":
            return (
                inputs + outputs,
                torch.nn.functional.softplus(grad.squeeze()).sum(-1),
            )
        elif self.res == "gated":
            return (
                self.gate.sigmoid() * outputs
                + (1 - self.gate.sigmoid()) * inputs,
                (
...
class MaskedWeight(torch.nn.Module):
    ...
    def get_weights(self):
        ...
        wpl = (
            self._diag_weight + self._weight - 0.5 * torch.log(w_squared_norm)
        )

        return (
            w.t(),
            wpl.t()[self.mask_d.bool().t()].view(
                self.dim,
...
class Tanh(torch.nn.Tanh):
    ...
    def forward(self, inputs, grad: torch.Tensor = None):
        ...
        g = -2 * (
            inputs - math.log(2) + torch.nn.functional.softplus(-2 * inputs)
        )
        return (
            torch.tanh(inputs),
